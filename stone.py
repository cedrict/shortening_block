...
for iter in range(0,niter):
   ...
   for iel in range(0,nel):
       ...
       for jq in [0,1,2]:
           for iq in [0,1,2]:
               ...
               etaq[counter]=viscosity(exxq,eyyq,exyq,pq[counter],cohesion,phi,\
   ...
   ######################################################################
   # removing nullspace by prescribing pressure at top right corner
   ######################################################################
   ...
   convfile.write("%3d %10e %10e %10e %10e %10e\n" %(iter,conv_two,conv_two,Ru,Rv,Rp)) 
   convfile.flush()
   ...
   print("compute elt press & sr: %.3f s" % (timing.time() - start))
   ...
   for iel in range(0,nel):
       for iq in [0,1,2]:
           for jq in [0,1,2]:
               ...
               avrg_sr+=srq[counterq]*weightq*jcob
               counterq+=1
           #end for
       #end for
   #end for
   ...
   if iter%1==0:
      ...
      vtufile.write("</DataArray>\n")
      vtufile.write("<DataArray type='Float32' Name='strain_rate' Format='ascii'> \n")
      for iq in range(0,nq):
          vtufile.write("%10e \n" % srq[iq])
      vtufile.write("</DataArray>\n")
...
vtufile.write("<DataArray type='Float32' Name='viscosity' Format='ascii'> \n")
for iel in range (0,nel):
    eta= viscosity(exx[iel],eyy[iel],exy[iel],pc[iel],cohesion,phi,iter,xc[iel],yc[iel],eta_m,eta_v)
    vtufile.write("%10e\n" %eta) 
vtufile.write("</DataArray>\n")
#--
